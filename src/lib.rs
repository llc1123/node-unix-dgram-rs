...
//     #[napi]
//     pub async fn send(&self, buffer: Buffer) -> napi::Result<i64> {
//         let buf: Vec<u8> = buffer.into();
//         Ok(self.datagram.send(&buf).await? as i64)
//     }

//     /// Sends data on the socket to the specified address.
//     #[napi]
//     pub async fn send_to(&self, buffer: Buffer, address: String) -> napi::Result<i64> {
//         let buf: Vec<u8> = buffer.into();
...
#[napi]
pub fn create_socket<F>(callback: F) -> napi::Result<UnixDatagram>
where
    F: Fn(Buffer, RemoteInfo) -> Result<()>,
{
    Ok(UnixDatagram {
        _buffer_size: BUFFER_SIZE,
